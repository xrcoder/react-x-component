    ...
    _createClass(XUpload, [{
        ...
        value: function uploadFiles(files) {
            ...
            postList.map(function (item, index) {
                ...
                if (sync) {
                    //多文件 同步上传，串行
                    promiseList.push(_this3.createUploadPromiseList(file, index + 1));
                } else {
                    // 异步上传，并行
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    _this4.uploadList[uid] = (0, _upload2.default)({
                        ...
                        onSuccess: function onSuccess(res, xhr) {
                            ...
                            resolve({ status: 'ok', index: index, file: file, res: res, xhr: xhr }); // index表示当前进度,status表示上传是否成功
                        },
                        onError: function onError(err, res) {
                            resolve({ status: 'fail', index: index, file: file, res: res, xhr: null });
                        }
                    });
                });
            };
